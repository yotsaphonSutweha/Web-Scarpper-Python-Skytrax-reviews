...
try:
    with open('skytrax_reviews.csv', 'w') as csv_file:
        ...
        csv_writer.writerow(['review_title', 'publish_date', 'overall_rating', 'reviewer_name', 'review_comment', 'aircraft', 'traveller_type', 'seat_type', 'route', 'date_flown', 'recommend', 'seat_comfort_rating', 'cabin_service_rating', 'food_and_beverages_rating', 'inflight_entertainment_rating', 'ground_service_rating', 'wifi_connectivity_rating', 'value_for_money_rating'])
        for number in (range(12,16)):
            response = requests.get('https://www.airlinequality.com/airline-reviews/delta-air-lines/page/' + str(number) + '/?sortby=post_date%3ADesc&pagesize=100')
            soup = BeautifulSoup(response.text, 'html.parser')
            reviews = soup.select('.list-item')
            for review in reviews:
                title = review.find(class_='text_header').getText()
                for i in review.findAll('time'):
                    if i.has_attr('datetime'):
                        publishDate = i['datetime']
                        x = re.search('^2014', publishDate)
                        if (x):
                            csv_file.close()
                if(review.find(attrs={"itemprop": "ratingValue"}) != None):   
                ...
                for reviewRating in reviewRatings:
                    ...
                    if(len(reviewRating.findAll(class_="review-value")) == 6):
                        ...
                        print(recommended)
                        print('-----------------------------------')
                    elif(len(reviewRating.findAll(class_="review-value")) == 2):
                        seatType = reviewRating.findAll(class_="review-value")[0].getText()
                        recommended = reviewRating.findAll(class_="review-value")[1].getText()
                        print(typeOfTraveller)
                        print(seatType)
                        print(route)
                        print(dateFlown)
                        print(recommended)
                        print('-----------------------------------')
                csv_writer.writerow([title, publishDate, overallRating, reviewerName, reviewComment, aircraft, typeOfTraveller, seatType, route, dateFlown, recommended, seatComfort, cabinService, foodAndBeverages, entertainment, groundService, wificonnectivity, valueForMoney])
                seatComfort = 0
                cabinService = 0
                foodAndBeverages = 0
                entertainment = 0
